...
class RunTestCommand extends CreateProductsCommand {
	...
	protected function createCustomer( $email ) {
		...
		$orderData = [
			'currency_id'      => 'USD',
			'email'            => $email !== null ? $email : 'cristian' . rand( 0, 1000 ) . '@itwebexperts.com',
			//buyer email id
			'shipping_address' => [
	...
	protected function execute( InputInterface $input, OutputInterface $output ) {
		...
		foreach ( $testSuite as $suite ) {
			...
			foreach ( $tests as $test ) {
				...
				if ( $test['type'] == 'createCustomer' ) {
					$parameters = [];
					eval( "\$parameters = {$test['arguments']};" );
					$orderId = $this->createCustomer( $parameters );
					if ( array_key_exists( 'return', $test ) ) {
						${$test['return']['@attributes']['variable']} = $orderId;
					}
				}
				...
				if ( $test['type'] == 'function' ) {
					...
					if ( array_key_exists( 'return', $test ) ) {
						${$test['return']['@attributes']['variable']} = call_user_func_array( [
							$class,
							$functionName,
						], $parameters );
